 ...
 *
 * \file      OranSupervisionHandler.cpp
...
#error "OranSupervisionHandler: Invalid default interval"
#endif
#if (CU_MONITORING_INTERVAL_MIN > CU_MONITORING_INTERVAL_MAX)
#error "OranSupervisionHandler: Invalid interval range"
#endif
#if (CU_MONITORING_INTERVAL_MAX < 1)
#error "OranSupervisionHandler: Invalid maximum interval"
#endif
...
//-------------------------------------------------------------------------------------------------------------
OranSupervisionHandler::OranSupervisionHandler(std::shared_ptr<IYangModuleMgr> moduleMgr)
	: YangHandlerSysrepo(moduleName, moduleMgr, "OranSupervisionHandler")
...
//-------------------------------------------------------------------------------------------------------------
OranSupervisionHandler::~OranSupervisionHandler()
{
}

//-------------------------------------------------------------------------------------------------------------
bool OranSupervisionHandler::initialise()
{
	// ------------------------------------------------------------------------
...
// #endif /* SUPPORT_CU_MONITORING */
	...
	// +--x supervision-watchdog-reset
	REGISTER_RPC("supervision-watchdog-reset", OranSupervisionHandler::rpcSupervisionWatchdogReset);
...
//-------------------------------------------------------------------------------------------------------------
void OranSupervisionHandler::valueChange(const std::string & xpath,
                                                         std::shared_ptr<YangParam> oldValue,
                                                         std::shared_ptr<YangParam> newValue)
{
	logDebugVerbose("OranSupervisionHandler::valueChange() - '%s'", xpath.c_str());
	...
	else
	{
		...
		if (leaf == "configured-cu-monitoring-interval")
		{
			...
			{
				eventError("OranSupervisionHandler - '%s'", error.c_str());
			}
		}
	}
}
...
//-------------------------------------------------------------------------------------------------------------
bool OranSupervisionHandler::rpcSupervisionWatchdogReset(std::shared_ptr<sysrepo::Session> session,
...
//-------------------------------------------------------------------------------------------------------------
bool OranSupervisionHandler::getRpcArg(std::shared_ptr<YangParams> params,
...
//-------------------------------------------------------------------------------------------------------------
void OranSupervisionHandler::netconfSessionUpNotification()
{
